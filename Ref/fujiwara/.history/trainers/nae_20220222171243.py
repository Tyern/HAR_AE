...
class NAETrainer(BaseTrainer):
    def train(self, model, opt, d_dataloaders, logger=None, logdir='', scheduler=None, clip_grad=None):
        ...
        print(len(indist_train_loader))
        # print(len(indist_val_loader))
        # print(len(oodval_val_loader))
        # print(len(oodtarget_val_loader))
        ...
        if cfg.get('fix_D', False):
            ...
            l_params = [{'params': list(model.encoder.parameters()) + list(model.decoder.parameters())}] #listの+は末尾に追加される
        ...
        #     print('epoch start')
        #     # indist_train_loderはbatchをいくつも持っていて，forで与えられるx,yは1つのbatchのデータ（batch_size個）とそのラベル
        #     for x, y in indist_train_loader:
        #         #print(y)
        #         i += 1
        #         print(i)
        #         model.train() #batchnormやdropoutは学習時のみに使うためこれを言ってあげる
        ...
        #         start_ts = time.time()
        #         d_train = model.train_step_ae(x, ae_opt, i_epoch, clip_grad=0.1)  # todo: clip_grad
                
        #         time_meter.update(time.time() - start_ts)
        #         logger.process_iter_train(d_train) # これまでのbatchのlossの平均を出せる
        ...
        #             model.eval() 
        #             for val_x, val_y in indist_val_loader: # 検証用データセット
        #                 val_x = val_x.to(self.device, dtype=torch.float)
        ...
        #         if i_epoch == (n_ae_epoch-1):
        #             self.save_model(model, logdir, i_iter=i, i_epoch= i_epoch)
                
        
        # 学習が終わったAEに対して，周波数が異なる時系列データごとの再構成誤差を評価
        # loss_dict = {}
        
        # print(indist_train_loader.dataset[0][0]) # data
        # print(indist_train_loader.dataset[0][1]) # label
        # print(indist_train_loader.dataset[1][0])
        # print(indist_train_loader.dataset[0:10000][0])

        # x = indist_train_loader.dataset[0:10000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['2'] = model.error_validation(x)            0.0014
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[10000:20000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['4'] = model.error_validation(x)            0.0018
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[20000:30000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['8'] = model.error_validation(x)            0.0012
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[30000:40000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['16'] = model.error_validation(x)           0.0021
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[40000:50000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['32'] = model.error_validation(x)           0.0024
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[50000:60000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['64'] = model.error_validation(x)           0.0020
        # del x
        # torch.cuda.empty_cache()
        # x = indist_train_loader.dataset[60000:70000][0]
        # x = torch.from_numpy(x.astype(np.float)).clone()
        # x = x.to(self.device, dtype=torch.float)
        # loss_dict['128'] = model.error_validation(x)          0.0013
        # 学習が終わったAEに対して，周波数が異なる時系列データごとの再構成誤差を評価
        loss_dict = {}
        ...
        print(indist_train_loader.dataset[0][1])
        print(indist_train_loader.dataset[10000][1])
        print(indist_train_loader.dataset[20000][1])
        print(indist_train_loader.dataset[30000][1])

        # suffle = False にしてある！！
        x = indist_train_loader.dataset[0:10000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['2'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[10000:20000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['4'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[20000:30000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['8'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[30000:40000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['16'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[40000:50000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['32'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[50000:60000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['64'] = model.error_validation(x)
        del x
        torch.cuda.empty_cache()
        x = indist_train_loader.dataset[60000:70000][0]
        x = torch.from_numpy(x.astype(np.float)).clone()
        x = x.to(self.device, dtype=torch.float)
        loss_dict['128'] = model.error_validation(x)
        ...
        #     if str(y) not in loss_dict.keys():
        #         loss_dict[str(y)] = model.error_validation(x)
        #         print(loss_dict[str(y)])
        #     else:
        #         loss_dict[str(y)] += model.error_validation(x)
        #         print(loss_dict[str(y)])
        # print(loss_dict)
        ...
        # #     for x, _ in indist_train_loader:
        # for i_epoch in tqdm(range(n_nae_epoch)): # 進行状況のプログレスバーを表示
