...
class ConvNet1D(nn.Module):
    def __init__(self, in_chan=6, out_chan=64, out_activation='linear'):
        super().__init__()
        #デフォルト
        # self.conv1 = nn.Conv1d(in_chan, 32, kernel_size = 60, stride = 2)
        # self.conv2 = nn.Conv1d(32, 64, kernel_size = 61, stride = 2)
        # self.conv3 = nn.Conv1d(64, 128, kernel_size = 61, stride = 2)
        # self.conv4 = nn.Conv1d(128, 256, kernel_size = 11, stride = 2)
        # self.conv5 = nn.Conv1d(256, out_chan, kernel_size = 1, stride = 1)
        #layers = [self.conv1,
                #   nn.ReLU(),
                #   self.conv2,
                #   nn.ReLU(),
        ...
        self.conv1 = nn.Conv1d(in_chan, 128, kernel_size = 8, stride = 4, padding= 6)
        self.conv2 = nn.Conv1d(128, 128, kernel_size = 8, stride = 4)
        self.conv3 = nn.Conv1d(128, 128, kernel_size = 8, stride = 4)
        self.conv4 = nn.Conv1d(128, 128, kernel_size = 8, stride = 4)
        layers = [self.conv1,
                  nn.ReLU(),
                  self.conv2,
                  nn.ReLU(),
    ...
    def forward(self, x):
        zero = torch.zeros(x.size(0), 128, 1, device='cuda:0')
        x = self.conv1(x)
        x = torch.relu(x)
...
class DeConvNet1D(nn.Module):
    def __init__(self, in_chan=64, out_chan=6, out_activation='linear', activation='relu',
                 num_groups=None, use_bn=False):
        super().__init__()
        # デフォルト
        # self.conv1 = nn.ConvTranspose1d(in_chan, 256, kernel_size=1, stride=1)
        # self.conv2 = nn.ConvTranspose1d(256, 128, kernel_size=11, stride=2)
        # self.conv3 = nn.ConvTranspose1d(128, 64, kernel_size=61, stride=2)
        # self.conv4 = nn.ConvTranspose1d(64, 32, kernel_size=61, stride=2)
        # self.conv5 = nn.ConvTranspose1d(32, out_chan, kernel_size=60, stride=2)
        ...
        self.conv2 = nn.ConvTranspose1d(128, 128, kernel_size=8, stride=4)
        self.conv3 = nn.ConvTranspose1d(128, 128, kernel_size=8, stride=4)
        self.conv4 = nn.ConvTranspose1d(128, 128, kernel_size=8, stride=4)
        self.conv5 = nn.ConvTranspose1d(128, out_chan, kernel_size=8, stride=4, padding=6)
        layers = [self.conv2,
                  nn.ReLU(),
                  self.conv3,
                  nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = torch.relu(x)
        # print(x.shape)
        x = torch.split(x, 7, dim=2)[0]
        # print('->',x.shape)
        x = self.conv3(x)
        x = torch.relu(x)
        # print(x.shape)
        x = torch.split(x, 31, dim=2)[0]
        # print('->',x.shape)
        x = self.conv4(x)
        x = torch.relu(x)
        # print(x.shape)
        x = torch.split(x, 127, dim=2)[0]
        # print('->',x.shape)
        x = self.conv5(x)
        print(x.shape)
        return x
...
class ConvNet2FC(nn.Module): # conv28のEncoder
...
class DeConvNet2(nn.Module): # Conv28のdecoder
...
class IGEBMEncoder(nn.Module): #Conv32BigのEncoder
    ...
    # ニューラルネットにデータを入れて出力を出す？　例えばエンコーダならxを入力してzを出す？
