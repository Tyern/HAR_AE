...
class ConvNet1D(nn.Module):
    def __init__(self, in_chan=3, out_chan=64, out_activation='linear'):
        ...
        self.conv4 = nn.Conv1d(128, 256, kernel_size = 8, stride = 4)
        layers = [self.conv1,
                  nn.ReLU(),
                  self.conv2,
                  nn.ReLU(),
    ...
    def forward(self, x):
        # zero = torch.zeros(128, 128, 1)
        x = self.conv1(x)
        print(x.shape)
        x = torch.relu(x)
        x = torch.cat([x, torch.zeros(128, 128, 1)], dim=2)
        print('->',x.shape)
        x = self.conv2(x)
        print(x.shape)
        x = torch.relu(x)
...
class DeConvNet1D(nn.Module):
    def __init__(self, in_chan=64, out_chan=3, out_activation='linear', activation='relu',
        ...
        self.conv2 = nn.ConvTranspose1d(256, 128, kernel_size=58, stride=2)
        self.conv3 = nn.ConvTranspose1d(128, 64, kernel_size=7, stride=2)
        self.conv4 = nn.ConvTranspose1d(64, 32, kernel_size=7, stride=2)
        self.conv5 = nn.ConvTranspose1d(32, out_chan, kernel_size=8, stride=2)
        layers = [self.conv2,
                  nn.ReLU(),
                  self.conv3,
                  nn.ReLU(),
