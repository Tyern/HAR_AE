...
class ConvNet1D(nn.Module):
    def __init__(self, in_chan=6, out_chan=64, out_activation='linear'):
        ...
        self.conv1 = nn.Conv1d(in_chan, 256, kernel_size = 8, stride = 4, padding= 6)
        self.conv2 = nn.Conv1d(256, 256, kernel_size = 8, stride = 4)
        self.conv3 = nn.Conv1d(256, 256, kernel_size = 8, stride = 4)
        self.conv4 = nn.Conv1d(256, 256, kernel_size = 8, stride = 4)
        layers = [self.conv1,
                  nn.ReLU(),
                  self.conv2,
                  nn.ReLU(),
    ...
    def forward(self, x):
        zero = torch.zeros(x.size(0), 256, 1, device='cuda:0')
        x = self.conv1(x)
        x = torch.relu(x)
...
class DeConvNet1D(nn.Module):
    def __init__(self, in_chan=64, out_chan=6, out_activation='linear', activation='relu',
        ...
        self.conv2 = nn.ConvTranspose1d(256, 256, kernel_size=8, stride=4)
        self.conv3 = nn.ConvTranspose1d(256, 256, kernel_size=8, stride=4)
        self.conv4 = nn.ConvTranspose1d(256, 256, kernel_size=8, stride=4)
        self.conv5 = nn.ConvTranspose1d(256, 6, kernel_size=8, stride=4, padding=6)
        layers = [self.conv2,
                  nn.ReLU(),
                  self.conv3,
                  nn.ReLU(),
