...
class FFEBM(nn.Module):
    ...
    def sample(self, x0=None, n_sample=None, device=None, replay=None):
        ...
        """
        print('nae.sample pass')
        #print(x0,n_sample,device,replay) # x0にx（batch）が入る
        if x0 is not None:
            n_sample = len(x0) # 1batchの画像の枚数
            device = x0.device
        if replay is None:
            replay = self.replay

        #print(self.sampling) #'on_manifold'
        if self.sampling == 'x': 
    ...
    def initial_sample(self, n_samples, device):
        l_sample = []
        if not self.replay or len(self.buffer) == 0: #最初は何もpushしてないのでlen(self.buffer)は0
            n_replay = 0
        else:
            n_replay = (np.random.rand(n_samples) < self.replay_ratio).sum() #n_samples回生成した0~1の少数がreplay_ratioを越えなかった回数
            l_sample.append(self.buffer.get(n_replay))

        shape = (n_samples - n_replay,) + self.sample_shape #(32, 1, 1)->(6, 32, 1, 1)など，複数枚のようになっている
...
class NAE(FFEBM):
    ...
    def error(self, x, recon):
        ...
        print(((x - recon) ** 2))
        print('----------------------------')
        print((x - recon) ** 2).view((x.shape[0], -1))
        print('----------------------------')
        print(((x - recon) ** 2).view((x.shape[0], -1)).sum(dim=1))
        print('----------------------------')
        return ((x - recon) ** 2).view((x.shape[0], -1)).sum(dim=1)

    def forward(self, x):
        """ Computes error per dimension """
        D = np.prod(x.shape[1:]) # [64, 3, 32, 32]の[3, 32, 32]をprodで掛け算 ,この場合3072
        z = self.encode(x) # エンコード
        recon = self.decoder(z) # デコード
        return self.error(x, recon) / D #64個の画像の平均二乗誤差を返す．
    ...
    def normalize(self, z):
        ...
        if self.spherical:
            #print(z.shape) #-> torch.Size([64, 32, 1, 1])
            if len(z.shape) == 4:
                z = z / z.view(len(z), -1).norm(dim=-1)[:, None, None, None]
            elif len(z.shape) == 3:
                z = z / z.view(len(z), -1).norm(dim=-1)[:, None, None]
            else:
    ...
    def encode(self, x):
        if self.spherical:
            # self.encoder(x)はIGEBMEncoderのforward(input)を呼び出していた
    ...
    def sample_omi(self, n_sample, device, replay=False):
        ...
        z0 = self.initial_sample(n_sample, device) # 初期値をサイコロを振る
        if self.spherical:
    ...
    def error_validation(self, x):
        print(x.shape)
        z = self.encode(x)
        print(z.shape)
        recon = self.decoder(z)
        x_dim = np.prod(x.shape[1:])
        recon_error = self.error(x, recon).mean() / x_dim #再構成誤差
        
        x_np = x.to('cpu').detach().numpy().copy()
        recon_np = recon.to('cpu').detach().numpy().copy()
        plt.plot(x_np[0][0])
        plt.plot(x_np[1][0])
        plt.plot(x_np[2][0])
        # plt.plot(recon_np[0][0])
        plt.show()
        
        return recon_error

    def train_step_ae(self, x, opt, i, clip_grad=None, y = None):
        opt.zero_grad() #勾配を0に設定
        z = self.encode(x)
        recon = self.decoder(z)
        x_np = x.to('cpu').detach().numpy().copy()
        recon_np = recon.to('cpu').detach().numpy().copy()
        ...
        recon_error = self.error(x, recon).mean() / x_dim #再構成誤差 x_dimはMNISTなら1*28*28の値
        print(recon_error)
        loss = recon_error


        # weight regularization 正則化で誤差関数の正則化（ペナルティ付け）
        ...
        if clip_grad is not None:
            torch.nn.utils.clip_grad_norm_(opt.param_groups[0]['params'], max_norm=clip_grad) # 勾配の上限を決めて勾配爆発を防ぐ
        opt.step() # 重み更新
        if y != None:
            d_result = {'loss': loss.item(), 'z_norm': z_norm.item(), 'recon_error_': recon_error.item(),
                        'decoder_norm_': decoder_norm.item(), 'encoder_norm_': encoder_norm.item(),'cycle':y}
        else:
            d_result = {'loss': loss.item(), 'z_norm': z_norm.item(), 'recon_error_': recon_error.item(),
    ...
    def train_step(self, x, opt):
        # print(self.x_shape)#None
        # print(self.z_shape)#None

        self._set_z_shape(x)
        self._set_x_shape(x)

        # print(self.x_shape)#[1, 28, 28]
        # print(self.z_shape)#[32, 1, 1]
        ...
        loss = (pos_e.mean() - neg_e.mean()) #式(11)
