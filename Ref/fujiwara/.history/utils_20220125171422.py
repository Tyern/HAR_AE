...
def roc_btw_arr(arr1, arr2):
    true_label = np.concatenate([np.ones_like(arr1), #
                                 np.zeros_like(arr2)])
    score = np.concatenate([arr1, arr2]) #距離　arr1:ood arr2:in_pred
...
def batch_run(m, dl, device, flatten=False, method='predict', input_type='first', no_grad=True, **kwargs):
    ...
    method = getattr(m, method) #getattr(m, method)はm.predictと等価
    
    l_result = []
    #DataLoderの持つ要素の1つ1つはバッチで，　1つのバッチ自体が1つのtensorであり，複数のtensor型のデータが入っている
    for batch in dl:
        #print(**kwargs)
        if input_type == 'first':
            x = batch[0] #RGBの3チャンネルを持つ画像を複数(64個とか)まとめたもの
        ...
        if no_grad:
            ...
            if flatten:
                x = x.view(len(x), -1)
            #print(method(x.cuda(device),**kwargs))
            #x.cuda(device)はGPU上のbatchを指す？
            #**kwargsは空のdict？
            #print(m.predict)
            pred = method(x.cuda(device), **kwargs).detach().cpu() # 64枚分の二乗誤差
        print(pred)
        l_result.append(pred)
    return torch.cat(l_result) #listのpredの要素をすべて対応するとこで足し合わせる
...
def get_shuffled_idx(N, seed):
    ...
    return rng.permutation(N) #np.arange(N)の配列をランダムに並べ替えて返す
