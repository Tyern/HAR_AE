...
from loaders.leaveout_dataset import MNISTLeaveOut, CIFAR10LeaveOut, SHLLeaveOut
...
from loaders.artificial_dataset import Artifical
...
def get_dataloader(data_dict, mode=None, mode_dict=None, data_aug=None):
    ...
    # dataset loading
    aug = get_composed_augmentations(data_dict.get('augmentations', None)) #get関数で''内のキーの値が帰ってくる．キーが存在しないならNoneが帰ってくる．
    ...
    # dataloader loading
    loader = data.DataLoader( #データセットからデータをバッチサイズに固めて返すモジュール　データを返す時にはそれぞれがtensor型
        dataset,
...
def get_dataset(data_dict, split_type=None, data_aug=None, dequant=None):
    ...
    """
    do_concat = any([k.startswith('concat') for k in data_dict.keys()]) #kが'concat'という文字から始まっているkeyがあるか(evaluateには無い)
    if do_concat:
    ...
    original_data_aug = data_aug #evaluateではNone
    if data_aug is not None: 
    ...
    # datasets
    if name == 'artificial':  #人工データ
        dataset = Artifical(seed)
        print(dataset)
    elif name == 'SHLLeaveOut':  #SHL内の1クラスを分布外データとして行うもの
        l_use_class = data_dict['use_class']
        dataset = SHLLeaveOut(data_path, l_use_class=l_use_class, split=split_type)
        print(dataset)
    elif name == 'MNISTLeaveOut':  #これはMNIST内の1クラスを分布外データとして行うもの？
        l_out_class = data_dict['out_class']
        dataset = MNISTLeaveOut(data_path, l_out_class=l_out_class, split=split_type, download=True,
                                transform=data_aug)
    elif name == 'MNISTLeaveOutFragment': #これはMNISTLeaveOutで学習したモデルに対し評価を行うときに省いた1クラス分？
        l_out_class = data_dict['out_class']
        fragment = data_dict['fragment']
        dataset = MNISTLeaveOut(data_path, l_out_class=l_out_class, split=split_type, download=True,
                                transform=Compose([ToTensor(),
                                                   Fragment(fragment)]))    #torchvision.transforms.Compose はlistの要素を先頭から実行するもの
    elif name == 'MNIST_OOD':
        size = data_dict.get('size', 28) #デフォルトは28　--datasetが'MNIST_OOD'か'FashionMNIST_OOD'であれば28，それ以外はのデータセットは32
        if size == 28:
        ...
        size = data_dict.get('size', 28) #デフォルトは28　--datasetが'MNIST_OOD'か'FashionMNIST_OOD'であれば28，それ以外はのデータセットは32　→要は学習させたデータセットに合わせてOODと認識したいデータセットのサイズをいじくったりする
        if size == 28:
            l_transform = [ToTensor()]
        else: #--datasetが'MNIST_OOD', 'FashionMNIST_OOD'　以外
            print(__file__,'\n','--datasetがMNIST_OOD, FashionMNIST_OOD 以外です')
            l_transform = [Gray2RGB(), Resize(OOD_SIZE), ToTensor()] #データセットをいじくる操作をlistで定義し，Composeで実行

        #データセット読み込み
        dataset = FashionMNIST_OOD(data_path, split=split_type, download=True, #datasetをどういうモードで読み込むか指定(split='training'とか)
                            transform=Compose(l_transform)) #このtransformを使うにはtransform(ndarrayかPIL)の形でできる transformは関数の名前 データセットを参照するときに行う前処理を定義する
        dataset.img_size = (size, size)
