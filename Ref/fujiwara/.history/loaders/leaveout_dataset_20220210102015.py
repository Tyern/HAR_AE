...
class SHLLeaveOut(Dataset):
    ...
    def __init__(self, root, l_out_class, split='training', transform=None, target_transform=None,
        ...
        #data_LAcc_x = np.loadtxt(path_LAcc_x, delimiter=" ", dtype='float')
        start = time.time()
        #data_LAcc_x = pd.read_table(path_LAcc_x, delimiter=' ', header=None)
        df = ddf.read_csv(path_LAcc_x, sep = ' ', header = None)
        df_dask = df.compute()
        data_LAcc_x = df_dask.to_numpy()
        print(data_LAcc_x.dtypes)
        print(round(time.time() - start,5),'秒')
        print('load -> LAcc_x')
        data_LAcc_y = np.loadtxt(path_LAcc_y, delimiter=" ", dtype='float')
        print('load -> LAcc_y')
        data_LAcc_z = np.loadtxt(path_LAcc_z, delimiter=" ", dtype='float')
        print('load -> LAcc_z')
        data_Gyr_x = np.loadtxt(path_Gyr_x, delimiter=" ", dtype='float')
        print('load -> Gyr_x')
        data_Gyr_y = np.loadtxt(path_Gyr_y, delimiter=" ", dtype='float')
        print('load -> Gyr_y')
        data_Gyr_z = np.loadtxt(path_Gyr_z, delimiter=" ", dtype='float')
        print('load -> Gyr_z')
        print(data_LAcc_x)
