...
class FashionMNIST_OOD(FashionMNIST):
    def __init__(self, root, split='training', transform=None, target_transform=None,
        ...
        super(FashionMNIST_OOD, self).__init__(root, transform=transform,
                                        target_transform=target_transform, download=download)
        assert split in ('training', 'validation', 'evaluation') #assertでsplitがどれかに当てはまるかどうかチェック

        #学習用のデータを取ってくるかどうかを指定　evaluationの時にはもちろんいらないからFalse
        if split == 'training' or split == 'validation':
            self.train = True
            shuffle_idx = get_shuffled_idx(60000, seed) #array[5,58689,245,…]など
        else:
            self.train = False
        self.split = split
        ...
        if self.train:
            data_file = self.training_filefrom
        else:
            data_file = self.test_file
    ...
    @property
    def processed_folder(self):
        return os.path.join(self.root, 'FashionMNIST', 'processed') #self.rootにdatasets(evaluate_odd.pyのdata_dict[path]である)ため
