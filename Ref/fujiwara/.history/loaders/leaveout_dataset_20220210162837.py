    ...
    def __init__(self, root, l_out_class, split='training', transform=None, target_transform=None,
        ...
        for i in range(len(label_numpy)):
            ...
            if label_numpy[i][0]*500 != sum(label_numpy[i]):
                non_unique_label.append(i)
        print(non_unique_label)
        print('ラベルの切り替わり:',len(non_unique_label)) # 581個
        print(round(time.time() - start,5),'秒')

        label_numpy = np.delete(label_numpy, non_unique_label, axis=0)


        #線形加速度・角速度データ
        ndarray_list = np.empty((0,196072,1,500),float)
        i=1
        for path in path_list:
            ...
            data_numpy = np.reshape(data_numpy, (1,196072, 1, 500))
            print(round(time.time() - start,5),'秒')
        ...
        print(data_np)
        print(data_np.shape)
        data_np = np.delete(data_np, non_unique_label, axis=0)
        print(data_np.shape)
