...
class SHLLeaveOut(Dataset):
    ...
    def __init__(self, root, l_use_class, split='training', ):
        """
        ...
        for c in l_use_class:
            ...
            c_label_dict[c] = label_np[get_list]
        ...
        if split == 'training' or split == 'validation' or split == 'in_validation' or split == 'out_validation':
        ...
        if split == 'training':
            ...
            acc_data = data[:,[0,1,2],:]
            gyr_data = data[:,[3,4,5],:]
            SHLLeaveOut.acc_mean = np.mean(acc_data)
            SHLLeaveOut.gyr_mean = np.mean(gyr_data)
            SHLLeaveOut.acc_std = np.std(acc_data)
            SHLLeaveOut.gyr_std = np.std(gyr_data)

        elif split == 'in_validation':
            for c, dataset in c_dataset_dict.items():
                test_dataset_dict[c] = dataset[10000:15000]
            for c, label in c_label_dict.items():
                test_label_dict[c] = label[10000:15000]
            for c, dataset in test_dataset_dict.items():
                data = np.append(data, dataset, axis=0)
            print(data.shape)
            for c, label in test_label_dict.items():
                targets = np.append(targets, label, axis=0)
            print(targets.shape)
        elif split == 'out_validation':
            for c, dataset in c_dataset_dict.items():
                test_dataset_dict[c] = dataset[0:20000]
            for c, label in c_label_dict.items():
                test_label_dict[c] = label[0:20000]
            for c, dataset in test_dataset_dict.items():
                data = np.append(data, dataset, axis=0)
            print(data.shape)
            for c, label in test_label_dict.items():
        ...
        # 標準化
        print('標準化前')
        data[:,[0,1,2],:] = (data[:,[0,1,2],:] - self.acc_mean ) / self.acc_std
        data[:,[3,4,5],:] = (data[:,[3,4,5],:] - self.gyr_mean ) / self.gyr_std
        print(self.acc_mean,self.acc_std)
        print(self.gyr_mean,self.gyr_std)
        print('標準化後')
        print(data)
        print(data.shape) # (195491, 6, 1, 500)    
