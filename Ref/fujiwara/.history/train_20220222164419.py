...
def run(cfg, writer):
    ...
    for key, dataloader_cfg in cfg['data'].items(): # keyはz32.ymlのindist_trainなど，dataloder_cfgはそれに対応する辞書
        #if 'holdout' in cfg: # 設定ファイルにちゃんと記述すればなくてもよい
            # dataloader_cfg = process_holdout(dataloader_cfg, int(cfg['holdout']))
        print(key,dataloader_cfg)
...
if __name__ == '__main__':
    ...
    left = [2, 4, 8, 16, 32, 64, 128]  # 横軸(棒の左端の位置)
    height = [0.0014, 0.0018, 0.0012, 0.0021, 0.0024, 0.0020, 0.0013]  # 値
