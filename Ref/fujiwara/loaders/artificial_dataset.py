...
class Artifical(Dataset):
    def __init__(self, seed):
        ...
        #     df.to_csv('datasets/artifical.csv', mode='a', index=False, header=False)
        #     targets.append(T)
        #     count += 1
        #     print(count)
        #     print(T)
        ...
        data = np.loadtxt('datasets/artifical.csv', delimiter=',')
        targets = np.load('datasets/artificial.npy')
        data = np.reshape(data, (70000, 6, 500))
        print(data[0:5])
        print(targets.shape)
        print('-----------------------------------------------')
        self.data = data
