...
def get_net(in_dim, out_dim, **kwargs): #**kwargsは **model_cfg["encoder"]か同様のdecoder
    nh = kwargs.get("nh", 8) #辞書型のkey"nh"のvalueを返す．なければ"8"を返す
    out_activation = kwargs.get("out_activation", "linear")

    if kwargs["arch"] == "conv1d":
    ...
    elif kwargs["arch"] == "deconv1d":
        net = DeConvNet1D(
            in_chan=in_dim, out_chan=out_dim, out_activation=out_activation
        )
...
#NAEのネットワーク構造をエンコーダ・デコーダそれぞれ取得してモデルを返す
def get_nae(**model_cfg): #model_cfgはymlファイルから作ったdictの"model"の部分，ex)z32gn.yml
    arch = model_cfg.pop("arch") #archをmodel_cfgから削除
    x_dim = model_cfg["x_dim"]
    z_dim = model_cfg["z_dim"]

    #get_netの返り値は，ハイパーパラメータを指定してある，あるネットワーク構造のインスタンス
    #と，"nae"にある辞書型のデータ
...
# *は可変長引数（argsは１つでも２つで３つでもいい，複数ならタプルに格納される）
# **は同じく可変長だが，引数にはdict型の各keyとvalueとなる
def get_model(cfg, *args, version=None, **kwargs):
    ...
    model = model(**model_dict) #get_nae(**model_dict)
    return model
