...
def read_datas(dir: str, car_ids: list, timestep: int, timeshift: int):
    ...
    labels["label"] = xlabel
    if len(labels) > 10000: labels = labels.sample(n = 10000, replace = True)
    yidxs = labels.index.values
    _labels = labels[["car_id", "label", "timestamp"]]
    _labels["date"] = np.datetime_as_string(labels.timestamp, "D")
    #_labels.to_csv(metapath, sep = "\t", index_label = "row_no")
    xlabel = np_utils.to_categorical(xlabel, num_classes = 4)
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(args.outeval, "wt") as f:
            ...
            for _ptn in file_ptns:
                for _label in glob.glob(_ptn, recursive = True):
                    ...
                    _edata, _elabel = read_csv(_accel, _gyro, _label, args.timestep, args.timeshift)
                    ...
                    eval = model.evaluate(_edata, _elabel, batch_size = args.batch_size, verbose = 1)
